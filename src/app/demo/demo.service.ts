...
export class DemoService {
  ...
  async seedDemoData(userId: string, expiresAt: Date): Promise<void> {
    ...
    const home1 = await this.homeService.create(
      {
        address: '42 Maple Drive, Springfield, IL',
        yearBuilt: 1998,
        sqFootage: 1800,
      },
      userId,
    ...
    const home2 = await this.homeService.create(
      {
        address: '88 Birchwood Court, Lakeside, OR',
        yearBuilt: 2005,
        sqFootage: 2200,
      },
      userId,
    ...
    const taskInputs = [
      ...
      },
      {
        ...
        frequency: HomeTaskFrequency.BI_ANNUAL,
      },
      {
        homeId: home1.id,
    ...
    const tasks = await Promise.all(
      taskInputs.map((t) => this.homeTaskService.create(t, userId, opts)),
    );
    ...
    await Promise.all(
      tasks.map(async (task) => {
        for (const date of completionDates) {
          await this.homeCompletionService.create(
            { taskId: task.id, date },
            userId,
            opts,
          );
